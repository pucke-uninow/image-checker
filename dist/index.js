...
exports.checkImageManifest = exports.parseImageString = exports.buildRegistryQueryUrl = exports.IMAGE_REGEX = void 0;
const core = __importStar(__nccwpck_require__(2186));
const http_client_1 = __nccwpck_require__(6255);
const fs_1 = __importDefault(__nccwpck_require__(7147));
const js_yaml_1 = __importDefault(__nccwpck_require__(1917));
const httpClient = new http_client_1.HttpClient();
exports.IMAGE_REGEX = /ghcr\.io\/(?<org>\w+)\/?(?<repo>\w+)?\/(?<imageName>\w+):(?<tag>.*)/;
function buildRegistryQueryUrl(org, repo, imageName, tag) {
    return `https://ghcr.io/v2/${org}${repo ? `/${repo}` : ''}/${imageName}/manifests/${tag}`;
}
exports.buildRegistryQueryUrl = buildRegistryQueryUrl;
function parseImageString(image) {
    const { 
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    groups: { org, repo, imageName, tag } } = image.match(exports.IMAGE_REGEX);
    if (!org || !imageName || !tag) {
        core.setFailed(`Could not parse Image string: ${image}`);
        return { org: '', repo: '', imageName: '', tag: '' };
    }
    return { org, repo: repo !== null && repo !== void 0 ? repo : null, imageName, tag };
}
exports.parseImageString = parseImageString;
async function checkImageManifest(url, token) {
    const encodedToken = Buffer.from(token).toString('base64');
    return httpClient.get(url, {
        Authorization: `Bearer ${encodedToken}`
    });
}
exports.checkImageManifest = checkImageManifest;
async function run() {
    var _a, _b, _c, _d;
    try {
